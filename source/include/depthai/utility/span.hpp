...
#define DAI_SPAN_NAMESPACE_NAME dai

#if __cplusplus >= 201703L || (defined(_MSVC_LANG) && _MSVC_LANG >= 201703L)
    #define DAI_SPAN_HAVE_CPP17
#endif

#if __cplusplus >= 201402L || (defined(_MSVC_LANG) && _MSVC_LANG >= 201402L)
    #define DAI_SPAN_HAVE_CPP14
#endif

namespace DAI_SPAN_NAMESPACE_NAME {

// Establish default contract checking behavior
#if !defined(DAI_SPAN_THROW_ON_CONTRACT_VIOLATION) && !defined(DAI_SPAN_TERMINATE_ON_CONTRACT_VIOLATION) && !defined(DAI_SPAN_NO_CONTRACT_CHECKING)
    #if defined(NDEBUG) || !defined(DAI_SPAN_HAVE_CPP14)
        #define DAI_SPAN_NO_CONTRACT_CHECKING
    #else
        #define DAI_SPAN_TERMINATE_ON_CONTRACT_VIOLATION
    #endif
#endif

#if defined(DAI_SPAN_THROW_ON_CONTRACT_VIOLATION)
...
#elif defined(DAI_SPAN_TERMINATE_ON_CONTRACT_VIOLATION)
[[noreturn]] inline void contract_violation(const char* /*unused*/) {
    std::terminate();
}
#endif

#if !defined(DAI_SPAN_NO_CONTRACT_CHECKING)
    #define DAI_SPAN_STRINGIFY(cond) #cond
    #define DAI_SPAN_EXPECT(cond) cond ? (void)0 : contract_violation("Expected " DAI_SPAN_STRINGIFY(cond))
#else
    #define DAI_SPAN_EXPECT(cond)
#endif

#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_inline_variables)
    #define DAI_SPAN_INLINE_VAR inline
#else
    #define DAI_SPAN_INLINE_VAR
#endif

#if defined(DAI_SPAN_HAVE_CPP14) || (defined(__cpp_constexpr) && __cpp_constexpr >= 201304)
    #define DAI_SPAN_HAVE_CPP14_CONSTEXPR
#endif

#if defined(DAI_SPAN_HAVE_CPP14_CONSTEXPR)
    #define DAI_SPAN_CONSTEXPR14 constexpr
#else
    #define DAI_SPAN_CONSTEXPR14
#endif

#if defined(DAI_SPAN_HAVE_CPP14_CONSTEXPR) && (!defined(_MSC_VER) || _MSC_VER > 1900)
    #define DAI_SPAN_CONSTEXPR_ASSIGN constexpr
#else
    #define DAI_SPAN_CONSTEXPR_ASSIGN
#endif

#if defined(DAI_SPAN_NO_CONTRACT_CHECKING)
    #define DAI_SPAN_CONSTEXPR11 constexpr
#else
    #define DAI_SPAN_CONSTEXPR11 DAI_SPAN_CONSTEXPR14
#endif

#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_deduction_guides)
    #define DAI_SPAN_HAVE_DEDUCTION_GUIDES
#endif

#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_lib_byte)
    #define DAI_SPAN_HAVE_STD_BYTE
#endif

#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_lib_array_constexpr)
    #define DAI_SPAN_HAVE_CONSTEXPR_STD_ARRAY_ETC
#endif

#if defined(DAI_SPAN_HAVE_CONSTEXPR_STD_ARRAY_ETC)
    #define DAI_SPAN_ARRAY_CONSTEXPR constexpr
#else
    #define DAI_SPAN_ARRAY_CONSTEXPR
#endif

#ifdef DAI_SPAN_HAVE_STD_BYTE
using byte = std::byte;
#else
using byte = unsigned char;
#endif

#if defined(DAI_SPAN_HAVE_CPP17)
    #define DAI_SPAN_NODISCARD [[nodiscard]]
#else
    #define DAI_SPAN_NODISCARD
#endif

DAI_SPAN_INLINE_VAR constexpr std::size_t dynamic_extent = SIZE_MAX;
...
#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_lib_nonmember_container_access)
using std::data;
using std::size;
#else
template <class C>
...
#endif  // DAI_SPAN_HAVE_CPP17

#if defined(DAI_SPAN_HAVE_CPP17) || defined(__cpp_lib_void_t)
using std::void_t;
#else
template <typename...>
using void_t = void;
...
class span {
   ...
   public:
    ...
    DAI_SPAN_CONSTEXPR11 span(pointer ptr, size_type count) : storage_(ptr, count) {
        DAI_SPAN_EXPECT(extent == dynamic_extent || count == extent);
    }

    DAI_SPAN_CONSTEXPR11 span(pointer first_elem, pointer last_elem) : storage_(first_elem, last_elem - first_elem) {
        DAI_SPAN_EXPECT(extent == dynamic_extent || last_elem - first_elem == static_cast<std::ptrdiff_t>(extent));
    }
    ...
    template <typename T,
              std::size_t N,
              std::size_t E = Extent,
              typename std::enable_if<(E == dynamic_extent || N == E) && detail::is_container_element_type_compatible<std::array<T, N>&, ElementType>::value,
                                      int>::type = 0>
    DAI_SPAN_ARRAY_CONSTEXPR span(std::array<T, N>& arr) noexcept : storage_(arr.data(), N) {}

    template <
        typename T,
        std::size_t N,
        std::size_t E = Extent,
        typename std::enable_if<(E == dynamic_extent || N == E) && detail::is_container_element_type_compatible<const std::array<T, N>&, ElementType>::value,
                                int>::type = 0>
    DAI_SPAN_ARRAY_CONSTEXPR span(const std::array<T, N>& arr) noexcept : storage_(arr.data(), N) {}
    ...
    DAI_SPAN_CONSTEXPR_ASSIGN span& operator=(const span& other) noexcept = default;

    // [span.sub], span subviews
    template <std::size_t Count>
    DAI_SPAN_CONSTEXPR11 span<element_type, Count> first() const {
        DAI_SPAN_EXPECT(Count <= size());
        return {data(), Count};
    }
    ...
    DAI_SPAN_CONSTEXPR11 subspan_return_t<Offset, Count> subspan() const {
        DAI_SPAN_EXPECT(Offset <= size() && (Count == dynamic_extent || Offset + Count <= size()));
...
#ifdef DAI_SPAN_HAVE_DEDUCTION_GUIDES
...
#endif  // DAI_HAVE_DEDUCTION_GUIDES
...
class tuple_size<DAI_SPAN_NAMESPACE_NAME::span<ElementType, Extent>> : public integral_constant<size_t, Extent> {};

template <typename ElementType>
class tuple_size<DAI_SPAN_NAMESPACE_NAME::span<ElementType, DAI_SPAN_NAMESPACE_NAME::dynamic_extent>>;  // not defined

template <size_t I, typename ElementType, size_t Extent>
class tuple_element<I, DAI_SPAN_NAMESPACE_NAME::span<ElementType, Extent>> {
   public:
    static_assert(Extent != DAI_SPAN_NAMESPACE_NAME::dynamic_extent && I < Extent, "");
    using type = ElementType;
};

}  // end namespace std

#endif  // DAI_SPAN_HPP_INCLUDED
