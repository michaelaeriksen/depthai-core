...
static std::string serializeFdSet(const google::protobuf::Descriptor* toplevelDescriptor) {
    ...
    while(!toAdd.empty()) {
        ...
        for(int i = 0; i < next->dependency_count(); ++i) {
            ...
            // protobuf 6 returns absl::string_view from name(); the set is keyed by std::string
            // and its lookups are not transparent, so the conversion has to be explicit.
            if(seenDependencies.find(std::string(dep->name())) == seenDependencies.end()) {
                seenDependencies.insert(std::string(dep->name()));
                toAdd.push(dep);
            }
        }
    }
